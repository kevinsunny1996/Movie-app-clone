...
function recieveData(type,data) {
  return {
    type: type,
    data: data
  };
...
function requestData(type, page, searchKey='') {
  return dispatch => {
    if(!searchKey) {
      return dispatch(recieveData({ total: 0, items: [] }));
    }

    dispatch(recieveData({ total: 0, items:[] }));
    return fetch('http://www.omdbapi.com/?s=${searchKey}&page=${page}')
    .then(response => response.json()).then(json => dispatch(recieveData(type, {
        items:json.Search || [],
